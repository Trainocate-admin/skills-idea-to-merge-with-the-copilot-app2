...
- Any code that touches `localStorage` (or other browser-only APIs) must run behind a `client:load` hydration boundary, so server-side rendering of the static build never executes browser-only code.
